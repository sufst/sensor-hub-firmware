...
HAL_StatusTypeDef Sensors_Transmit(void)
{
    ...
    if (i2c_read(ISM330_ADDR, ISM330_OUT_A, buf, 6) != HAL_OK) { return HAL_ERROR; }
    memcpy(data, buf, 6);
    ...
    if (i2c_read(ISM330_ADDR, ISM330_OUT_G, buf, 6) != HAL_OK) { return HAL_ERROR; }
    memcpy(data, buf, 6);
    ...
    if (i2c_read(ISM330_ADDR, ISM330_OUT_TEMP, buf, 2) != HAL_OK) { return HAL_ERROR; }
    s_ism330_ok = 1U;
    data[0] = buf[0];
    data[1] = buf[1];

    /* MCP9800: 12-bit in upper bits of 16-bit register — right-shift to get signed 12-bit */
    if (i2c_read(MCP9800_ADDR, MCP9800_REG_T, buf, 2) != HAL_OK) { return HAL_ERROR; }
    s_mcp9800_ok = 1U;
